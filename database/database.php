...
function updateProduct($id, $tenSP, $maNCC, $maDM, $donGia, $soLuong, $moTa, $anhNen = '') {
    global $conn;
    $id = mysqli_real_escape_string($conn, $id);
    $tenSP = mysqli_real_escape_string($conn, $tenSP);
    $maNCC = mysqli_real_escape_string($conn, $maNCC);
    $maDM = mysqli_real_escape_string($conn, $maDM);
    ...
    $sql = "UPDATE sanpham SET 
            ...
            MoTa = '$moTa'";
    if($anhNen !== '') {
        $anhNen = mysqli_real_escape_string($conn, $anhNen);
        $sql .= ", AnhNen = '$anhNen'";
    }
    $sql .= " WHERE MaSP = '$id'";
    return mysqli_query($conn, $sql);
